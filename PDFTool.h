...
extern "C" {
  void initpdfm_(int& nset, int& imem);
  void initpdfsetbynamem_(int& nset, char name[], long len);
  void evolvepdfm_(int& nset, double& x, double& q, double f[13]);
  void fsilent_();
}
...
class PDFTool {
 public:
  PDFTool(double beam_energy, double reweightScale=1, int reweightPdf=-1, int init_pdfset=-1, int nmxset=56, bool silent=false)//i.e. beam_energy=3500000, init_pdfset=20650 (can be set later in event_weight)
   : m_q2(0), m_x1(0), m_x2(0), m_f1(0), m_f2(0) {
  ...
  double weight(int pdf=-1, double scale=1.){
    ...
    double pdf1org = testPdf(m_pdfset,m_x1,q,m_f1)/m_x1;

    // pdf of particle2 by original pdf
    double pdf2org = testPdf(m_pdfset,m_x2,q,m_f2)/m_x2;
    ...
    double weight = (pdf1new*pdf2new)/(pdf1org*pdf2org)/pow(scale,2);
    //std::cout << "pdf(p1)=" << pdf1org << " pdf(p2)=" << pdf2org << " -> pdf(p1)=" << pdf1new << " pdf(p2)=" << pdf2new << " weight=" << weight << std::endl;
  ...
  std::string pdfName(int pdfset){
    ...
    else if(pdfset==229800) return "NNPDF23_nlo_as_0118.LHgrid";
    else {
  ...
  int pdfMember(int pdfset){
    ...
    else if(pdfset==229800) return pdfset-229800;
    else {
  ...
  int setPdf(std::string pdfname, int member){
    for(unsigned int i=0; i<m_pdfsetNames.size(); i++){
      if(m_pdfsetNames[i]==pdfname && m_pdfsetMembers[i]==member) return i+1;
    }
    ...
    char temp[256];
    strcpy(temp,pdfname.c_str());
    initpdfsetbynamem_(nset,temp,pdfname.length());
